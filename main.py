...
def bybit_withdraw(address, amount_to_withdrawal, wallet_number):
    ...
    try:
        exchange.withdraw(
            ...
            address=address,
            tag=None,
            params={"forceChain": 1, "network": network},
        )
        print(
...
def choose_cex(address, amount_to_withdrawal, wallet_number):
    if switch_cex == "binance":
        ...
        print(
            f"\n>>> Bybit в больнице, у них API заболело, sorry"
        ...
        raise ValueError(
            "Неверное значение CEX. Поддерживаемые значения: binance, okx, bybit, gate, huobi, kucoin, mexc."
        )
...
def get_withdrawal_fee(symbolWithdraw, chainName):
    ...
    raise ValueError(
        f"     не могу получить сумму комиссии, проверьте значения symbolWithdraw и network"
    )
...
if __name__ == "__main__":
    with open("wallets.txt", "r") as f:
        ...
        for wallet_number, address in numbered_wallets:
            print(f"{count_wallet})")
            print("=" * 15)
            amount_to_withdrawal = round(
                random.uniform(amount[0], amount[1]), decimal_places
            )
